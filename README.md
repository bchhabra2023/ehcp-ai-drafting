...
|---|---|---|
| `AZURE_CLIENT_ID` | - | Optional client ID when you must target a **user-assigned** managed identity explicitly |
| `FOUNDRY_ENDPOINT` | - | Foundry-backed model endpoint, e.g. `https://<resource>.openai.azure.com/` |
| `FOUNDRY_PROJECT_NAME` | - | Foundry project name used to organise the deployment |
| `FOUNDRY_MODEL_NAME` | - | Deployed model name used by the app |
...
| `AZURE_STORAGE_ACCOUNT_URL` | - | Blob account URL used with managed identity |
...
| `AZURE_KEY_VAULT_URL` | - | Key Vault URL used by the frontend to read residual secrets at runtime |
| `ENTRA_CLIENT_SECRET_SECRET_NAME` | - | Secret name in Key Vault that stores the frontend app-registration secret |
...
- **Model choice** - change `FOUNDRY_MODEL_NAME`; keep `MODEL_TEMPERATURE=0` for reproducible
  extraction.
