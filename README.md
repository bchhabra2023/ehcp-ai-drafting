...
az cognitiveservices account create -n ehcp-openai -g $RG -l $LOC \
  --kind OpenAI --sku S0 --custom-domain ehcp-openai
az cognitiveservices account deployment create -n ehcp-openai -g $RG \
...
az cognitiveservices account create -n ehcp-docintel -g $RG -l $LOC \
  --kind FormRecognizer --sku S0 --custom-domain ehcp-docintel

# Storage account + container (optional but recommended)
az storage account create -n ehcpstorage$RANDOM -g $RG -l $LOC --sku Standard_LRS
az storage container create --account-name <storage-account> -n ehcp-outputs

# Cosmos DB for auditing (optional)
az cosmosdb create -n ehcp-cosmos -g $RG --locations regionName=$LOC
az cosmosdb sql database create -a ehcp-cosmos -g $RG -n ehcp-audit
az cosmosdb sql container create -a ehcp-cosmos -g $RG -d ehcp-audit \
  -n activity-logs --partition-key-path /session_id
az cosmosdb sql container create -a ehcp-cosmos -g $RG -d ehcp-audit \
  -n job-logs --partition-key-path /session_id

# Container Registry + Container Apps environment
az acr create -n <acrname> -g $RG --sku Basic --admin-enabled true
az containerapp env create -n cae-ehcp -g $RG -l $LOC
```

### Step 2 — (Optional) Register the Entra ID applications

1. Create a **backend API** app registration; expose an API with scope `user_impersonation` and
   note the Application (client) ID.
2. Create a **frontend** app registration; add a Web redirect URI matching the deployed frontend
   URL, create a client secret, and grant delegated permission to the backend API scope.
3. Set `AUTH_ENABLED=true` with the matching `ENTRA_*` values in both `.env` files.

### Step 3 — Configure environment files

Populate `backend/.env` (start from `backend/.env.example`) and `frontend/.env`. The deployment
script reads both files and injects the values into the Container Apps. Keys listed as secrets
(`AZURE_OPENAI_API_KEY`, `AZURE_DOCUMENT_INTELLIGENCE_KEY`, `AZURE_STORAGE_CONNECTION_STRING`) are
stored as Container App secrets and referenced via `secretref:`.

### Step 4 — Build and push the images

Set `$ACR_NAME` in `build-push.ps1`, then:

```powershell
.\build-push.ps1                      # tags as :latest
.\build-push.ps1 -Tag "1.0.0"         # or a semver / git SHA
```

This logs in to ACR and builds/pushes `ehcp-backend` and `ehcp-frontend`.

### Step 5 — Deploy the Container Apps

Set `$ACR_SERVER` in `deploy-aca.ps1` to your ACR login server, then:
...
- `ehcp-containerapp-backend` — target port 8000, **internal** ingress, 1–3 replicas
- `ehcp-containerapp-frontend` — target port 8501, **external** ingress, 1–3 replicas, with
  `BACKEND_URL` set automatically to the backend's internal FQDN

It prints both URLs on completion.

### Step 6 — Post-deployment steps

1. If using Entra ID, add the printed frontend URL as a redirect URI on the frontend app
   registration and set `ENTRA_REDIRECT_URI` accordingly, then redeploy.
2. If using managed identity (`USE_MANAGED_IDENTITY=true`), enable a system- or user-assigned
   identity on the backend Container App and grant it:
   ...
   Then remove the corresponding key values from the environment.
3. Smoke test the deployment by uploading the files under `Test Cases/Simple Case Inputs/` and
   comparing the draft with `Test Cases/Simple Case Output/`.
